...
but does not show any rotation (See Figure~\ref{fig:near_complete_rotation}).
...
There exists a numerous amount of methods to detect rotational activity \autocite{pikunov2023the, gurevich2019robust, li2020standardizing},
...
that phase barriers can have on the analysis.
...
In the second approach, we account for possible phase barriers,
...
Both approaches are applied to simulated activation patterns
to isolate different arising problems,
and to an experimental optical mapping recording of ventricular fibrillation in a rat heart.

\subsection{Setup of the Simulations}

As it is not required to have realistic geometries to induce phase barriers,
we chose to create simulations on a homogeneous 2D grid.
This also has the benefit to get a full overview of the simulation at once
and easily place snapshots in the paper.
The simulation is created using Finitewave (\url{https://github.com/finitewave/Finitewave}),
an open-source Python package for a wide range of tasks
in modelling cardiac electrophysiology using finite-difference methods.
The main argument for using Finitewave is its clear and transparent implementation
of the models which allows us to verify its correctness.
Furthermore, the intuitive interface and
lots of examples make it easy to create,
evaluate, and adjust simulations.

The cell model used for the simulation is the Fenton-Karma model.
Since the research question of this paper
does not involve the effect of ionic channels on the dynamics,
it is more fitting to use a phenomenological model
which is less computationally heavy and has simpler cell dynamics.
We choose the Fenton-Karma model specifically,
since it was the easiest to create phase defects.
All simulation are created starting from a 2D mesh
that is pre-paced with 10 planar stimuli
and an interval of 200 time steps between them.
To ensure that the rotor fits on the mesh,
but without increasing computational time,
we lowered the diffusion coefficient.

For each simulation the normalized action potential is tracked

\subsubsection{Complete and Near-Complete Rotation}

\subsubsection{Parallel Activity around a Phase Defect}

\subsubsection{Rotational Activity in Tissue with Diffuse Fibrosis}

\subsection{Optical Mapping of Ventricular Fibrillation}
...
The first step is to convert the action potential to a phase field.
This is done by using the Hilbert transform of the action potential
as suggested by \autocite{bray2002considerations}.
Additionally, the peak of the action potential corresponds to
...
Next, the mesh is tessellated
and compute the phase index for each at each time step.
For any cell, the phase index can be calculated by counting the number of phase jumps.
...
\subsection{Extended implementation of Phasemapping}
